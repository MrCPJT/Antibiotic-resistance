...
sa_co_df$mean <- sa_co_df[, 2]
sa_co_df[, 2:7] <- NULL
sa_co_df$mean <- rowMeans(sa_co_df[, 2], na.rm = TRUE)
sa_co_df <- na.omit(sa_co_df)

# Using individual replicate data - Comment out to measure averaged data
pa_co_df$mean <- pa_co_df[, 2]
pa_co_df[, 2:7] <- NULL
pa_co_df$mean <- rowMeans(pa_co_df[, 2], na.rm = TRUE)
pa_co_df <- na.omit(pa_co_df)
...
species_df = sa_co_df  # Species of interest for estimation
