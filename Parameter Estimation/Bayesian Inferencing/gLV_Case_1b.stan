...
// Case 1b - For generating growth intra-interactivity and re-estimating growth parameters
...
functions {
  real[] logisticgrowth(real t,
    ...
    real dudt = (1 - y[1]/Delta11 - y[2]/theta[3]) * y[1] * theta[1] ;
    real dvdt =  (1 - y[2]/Delta22 - y[1]/theta[4]) * y[2] * theta[2] ;
...
model {
  ...
  theta[1] ~ cauchy(0,10);     // Vague choice of priors
  theta[2] ~ cauchy(0,10);     // Vague choice of priors
  theta[3] ~ cauchy(0,100000); // Vague choice of priors
  theta[4] ~ cauchy(0,100000); // Vague choice of priors
