...
% Initial conditions
y(:,1) = ics(1,:);  % Initialising solution vector with initial conditions
yp(:,1) = ics(1,:); % Initialising solution vector for perturbed system with initial conditions

u = zeros((numel(te)-1),1);     % Initialising antibiotic dosage with initial conditions
u(5000) = 0;                   % Dosage at t = 1
...
% Title
title('11D gLV Time Series - CD-Susceptible','FontSize',14)

% Limits
ylim([0 26])

% Legend
leg = legend('','gLV', 'gLV + Perturbation','Location','northwest','FontSize',12);
leg.ItemTokenSize = [21,9];
title(leg,'Model')

% Annotation
text(305, 10.75, 'Infected D','color', 'k','FontSize',13)
text(60, 4.5, 'Uninfected B','color', 'k','FontSize',13)
text(205, 17, 'CD Inoculation','color', [0.4 0.4 0.4],'FontSize',13)
